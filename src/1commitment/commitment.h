...
public:
	//加密正确性证明
	Commitment(array<string, 2> codes, array<ZZ, 32> plaintext, array<Cipher_elg, 32> ciphertext, array<ZZ, 32> ran, bool bigMe, string fileName);
	//比较正确性证明
	Commitment(array<string, 2> codes, array<ZZ, 32> plaintext_1, array<Cipher_elg, 32> ciphertext_1, array<Cipher_elg, 32> ciphertext_2, array<ZZ, 32> ran_1, Cipher_elg cipherZero_1, Cipher_elg cipherZero_2, ZZ ranZero, bool bigMe, string fileName);
	//解密正确性证明
	Commitment(array<string, 2> codes, array<ZZ, 32> c2, array<ZZ, 32> dk, bool bigMe, string fileName);
	//验证
	Commitment(array<string, 2> codes, array<Cipher_elg, 32> ciphertext, bool bigMe, string fileName);
	//验证
	Commitment(array<string, 2> codes, bool bigMe, string fileName);
	//比较正确性证明
	void compareCommit();
	//比较正确性证明验证
