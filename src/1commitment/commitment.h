...
private:
	...
	array<Cipher_elg, 32> ciphertext_1;    //对方的密文
...
public:
	//加密正确性证明
	Commitment(array<string, 2> codes, array<ZZ, 32> plaintext, array<Cipher_elg, 32> ciphertext, array<ZZ, 32> ran, bool bigMe, string fileName) :codes(codes), plaintext(plaintext), ciphertext(ciphertext), ran(ran), bigMe(bigMe), fileName(fileName) {
	...
	//比较正确性证明
	Commitment(array<string, 2> codes, array<ZZ, 32> plaintext_1, array<Cipher_elg, 32> ciphertext_1, array<Cipher_elg, 32> ciphertext_2, array<ZZ, 32> ran_1, Cipher_elg cipherZero_1, Cipher_elg cipherZero_2, ZZ ranZero, bool bigMe, string fileName) :codes(codes), plaintext_1(plaintext_1), ciphertext_1(ciphertext_1), ciphertext_2(ciphertext_2), ran_1(ran_1), cipherZero_1(cipherZero_1), cipherZero_2(cipherZero_2), ranZero(ranZero), bigMe(bigMe), fileName(fileName) {
		mod = El.get_group().get_mod();
		ord = El.get_group().get_ord();
		g = El.get_group().get_g();
		h = El.get_group().get_gen();
